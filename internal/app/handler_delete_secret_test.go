...
func TestApplication_HandlerDeleteSecret(t *testing.T) {
	a := Application{
		...
		},
		{
			...
			input: input{
				...
				storage: func() storage.Storage {
					...
					s.
						EXPECT().
						DeleteSecret(mock.Anything, mock.Anything).
						Return(nil)
					return s
				},
			},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			r := httptest.NewRequest(
				http.MethodDelete,
				"/api/secret/2a9186b1-d39f-49cb-99a9-b6e8a25293a2",
				http.NoBody,
			)
